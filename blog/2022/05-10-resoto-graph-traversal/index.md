...
This search finds the DigitalOcean droplet with the related id, then traverses the graph outbound by one step, as defined by `-->` , and filters the resulting volume list. Other resources are pointing towards our droplet. By looking at the graph structure, we see it could be load balancers, Kubernetes clusters, firewalls, or images.
