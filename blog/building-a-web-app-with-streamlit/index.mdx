...
date: 2022-10-25
authors: [lukas]
tags: [Integrations, Metrics, Data Visualization, Search, Streamlit, AWS, Google Cloud]
description: Resoto cloud asset inventory data can be integerated with Streamlit, a framework that turns data into web apps.
---

# Building a Web App with Streamlit

In [Actionable Cloud Infrastructure Metrics](../actionable-cloud-infrastructure-metrics/index.mdx), we explored how to create metrics, export them into a time series database, and visualize them with Grafana. **Today, we'll take a look at how to build a web app using [Streamlit](https://streamlit.io), a framework that turns data into web apps.**

![Sheep looking inside a black box](./img/banner.png)

If you are not familiar with [Python](https://python.org), don't worry—we're going to keep it simple! In [Prerequisites](./index.mdx#prerequisites), we'll go over installing Python and the coding techniques utilized in this project.

{/* truncate */}

:::info

If you're already comfortable with [Python](https://python.org) and just want the finished product, you can follow the steps in [Environment Setup](./index.mdx#environment-setup) and jump straight to [The Final Product](./index.mdx#the-final-product). Then, see [Running the App](./index.mdx#running-the-app) for instructions on how to start the app.
...
If you already these prerequisites installed, you can skip ahead to [Environment Setup](./index.mdx#environment-setup).
...
This is an easy one. It's similar to the [demo app](./index.mdx#creating-a-demo-app), but instead of displaying the dataframe unchanged we use some of Panda's built-in functions to display the top 10 accounts and regions by on-demand cost.
...
We've covered the basics of building a web app with Resoto and [Streamlit](https://streamlit.io). Currently, we are only displaying data, but in my next post we will introduce input widgets to add interactive functions to the app.
