...
date: 2022-11-21
authors: [anja]
tags: [Resource Tagging, Automation, Search, Resoto CLI, AWS, Discord, Slack]
description: Cloud tagging strategies and policies are hailed as one of the most efficient ways to keep your cloud infrastructure controllable. Resoto offers service-agnostic the resource tag handling.
---
...
As highlighted in [Solving Cloud Resource Management Challenges with Resoto](../solving-cloud-resource-management-challenges/index.mdx), going through cloud infrastructure with a <abbr title="command-line interface">CLI</abbr> can be tiresome and requires contextual knowledge (because the API differs between services). This is hardly conducive to keeping tags under control.
...
For the sake of clarity, today's blog post only covers <abbr title="Amazon Web Services">AWS</abbr>. However, [Resoto treats resources from all cloud providers the same](../multi-cloud-resource-management/index.mdx) and you can easily customize the below search queries as needed.
...
Resoto allows you to define and schedule [jobs](/docs/concepts/automation#jobs) with the [`jobs add` command](/docs/reference/cli/job/add) to further reduce this kind of toil work:
...
If you would like to try it out today, you can install the `edge` version of Resoto (not recommended for use in production environments).
...
Because Resoto has a complete view over your entire [cloud asset inventory](../cloud-asset-inventory/index.mdx) **and** because Resoto is service-agnostic the tag handling even of millions of resources is no longer a fight against windmills but becomes a treat instead! ✨
