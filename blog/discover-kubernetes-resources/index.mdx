...
However, the above command only executes a single time. We would like for Resoto to constantly monitor for pods that are restarting too often and send a notification whenever such pods are found. To do so, we can create a [job](/docs/concepts/automation#jobs) using the [`job` command](/docs/reference/cli/action-commands/jobs):
