...
date: 2022-08-31
authors: [matthias]
tags: [Kubernetes, Search, Automation, Graph Database, Resoto CLI, Discord]
description: Kubernetes provides an API that returns information about a cluster's deployed resources. Resoto uses this API to build an index of resources in all of your Kubernetes clusters.
---
...
The examples presented in this blog post are available in the [`someengineering/resotonotebook` GitHub repository](https://github.com/someengineering/resotonotebook/blob/main/examples/resoto_kubernetes.ipynb).

Try them out on your own Kubernetes cluster after [installing Resoto](/docs/getting-started/install-resoto)!

:::

## Full-Text Search

**One of the simplest ways to find resources in your cluster is using Resoto's [full-text search](/docs/reference/search/full-text).**

Resoto indexes the properties of all services in every namespace in each cluster. To perform a full-text search, simply provide your search term to the [`search` command](/docs/reference/cli/search) enclosed in double quotes:
...
To understand the concept of a graph a bit better, let me show you two examples that we present in our [Jupyter notebook](/docs/reference/notebook) via `display_svg(rnb.graph(...))`:
...
We can do the same query, but this time reverse the direction of the arrow, showing us all incoming dependencies up to two levels. This time we see all resources that have a dependency on this service.

:::note

Resoto understands the underlying provider of this Kubernetes cluster, in this case, DigitalOcean.

This service type is `LoadBalancer`, so a publicly available load balancer needs to be provided. A DigitalOcean load balancer is used here, backing the Kubernetes service. You can also see that the DigitalOcean load balancer is provided in the Frankfurt region in the default VPC.

All of this knowledge is gathered automatically by Resoto without any additional action.
...
[Walking the relationships of a resource graph](../a-walk-in-the-graph/index.mdx) can reveal a lot of useful information.
...
[`count`](/docs/reference/cli/count) is actually an [aggregation](/docs/reference/search/aggregation). Aggregations apply functions like `min`, `max`, `avg`, and `sum` on the a resource's property values.
...
While aggregation can be helpful for adhoc queries, we can also turn this data into a time series and watch the values over time. See [Actionable Cloud Infrastructure Metrics](../actionable-cloud-infrastructure-metrics/index.mdx) for more details.
...
However, the above command only executes a single time. We would like for Resoto to constantly monitor for pods that are restarting too often and send a notification whenever such pods are found. To do so, we can create a [job](/docs/concepts/automation#jobs) using the [`job` command](/docs/reference/cli/job):
...
We registered this job with the [`post_collect` event](/docs/reference/events#collect-events), so it will be executed automatically whenever new data has been [collected](/docs/concepts/cloud-data-sync).

## Kubernetes… and More!

Resoto is not only able to collect resource metadata from Kubernetes, but also other cloud providers as well.
...
[Install Resoto](/docs/getting-started/install-resoto) today!
