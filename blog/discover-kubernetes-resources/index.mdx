...
Resoto indexes the properties of all services in every namespace in each cluster. To perform a full-text search, simply provide your search term to the [`search` command](/docs/reference/cli/search-commands/search) enclosed in double quotes:
...
However, the above command only executes a single time. We would like for Resoto to constantly monitor for pods that are restarting too often and send a notification whenever such pods are found. To do so, we can create a [job](/docs/concepts/automation#jobs) using the [`job` command](/docs/reference/cli/action-commands/job):
