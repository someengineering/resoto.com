...
date: 2023-06-06
authors: [matthias]
tags: [security, aws, resoto cli, resoto ui]
---
...
The first step in enforcing security compliance is to define a security baseline - a set of rules that all cloud resources must adhere to. The concept and rationale of security compliance and how to build a baseline have been introduced in my [previous blog post](../cloud-resource-security-benchmarks/index.mdx).
...
As we have seen in the previous blog post, Resoto allows you to define a security baseline and check your cloud resources against this baseline. ResotoUI has a gorgeous UI that allows you to easily navigate a security benchmark result, see affected cloud resources and get guidance on how to remediate the issue. Once all of your security-relevant cloud resources are compliant with your security baseline, you have a solid foundation for your cloud security.
...
We can not only write the result of a report to a file, but we could also take the result and send a message to a [Slack](/docs/how-to-guides/alerting/send-slack-notifications) or [Discord](/docs/how-to-guides/alerting/send-discord-notifications) channel. To make the message as precise and meaningful as possible, we define the account explicitly and only send the message if there are any issues:
...
> report benchmark run aws_cis_1_5
 ...
 --resource-id .reported.name
 --group-resources false
 --resource-properties {failing_resources: .reported.number_of_resources_failing}
```
