...
We've transitioned the `discord` and `slack` commands from [command templates](/docs/reference/cli/search-commands/templates) to [infrastructure apps](/docs/how-to-guides/infrastructure-apps/install-and-run-infrastructure-apps). Simply install them using the `app install discord` and `app install slack` commands. This allows us to keep them updated without the need to release a new version of Resoto.
