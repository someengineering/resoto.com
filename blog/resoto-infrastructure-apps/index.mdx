...
Nowadays, most organizations use tools like [<abbr title="Cloud Development Kit">CDK</abbr>](https://aws.amazon.com/cdk), [Terraform](https://terraform.io), or [CloudFormation](https://aws.amazon.com/cloudformation) to create their infrastructure following <abbr title="Infrastructure as Code">IaC</abbr> principles.
...
**We've introducing a powerful new feature in [Resoto 3.5](/releases/3.5.0): Resoto Infrastructure Apps.**
...
Finding and installing an Infrastructure App is a breeze the [`apps` command](/docs/reference/cli/miscellaneous-commands/apps), which allows you to search for and install apps.
...
In [Resoto 3.5](/releases/3.5.0), we've ported all existing Python plugins like the tag validator or abandoned load balancer cleaner to Infrastructure Apps.
...
With [`resotoappbundler`](https://github.com/someengineering/resotoappbundler), you can bundle and dry-run your app locally. `resotoappbundler` also allows you to bundle multiple Resoto app directories into a single JSON file that can be hosted on any HTTP server and used as an app index URL.

:::tip

See the [`someengineering/resoto-apps` GitHub repository](https://github.com/someengineering/resoto-apps) to review Resoto's built-in Infrastructure Apps.

:::

In addition to standard Jinja functions, Resoto Infrastructure Apps have access to several supplementary functions and variables, enabling complex app operations.
...
Whether it's pre-built or custom Infrastructure Apps, these low-code apps both simplify and amplify your infrastructure management capabilities. With such powerful tools at your disposal, managing cloud resources has never been easier!

## Further Reading

- [How to Install & Run Infrastructure Apps](/docs/how-to-guides/infrastructure-apps/install-and-run-infrastructure-apps)
- [`apps` Command](/docs/reference/cli/miscellaneous-commands/apps)
