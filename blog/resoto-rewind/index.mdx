...
Rewind introduces two new functions to Resoto: the [`graph` command](/docs/reference/cli/graph) and the `--at` flag on the [`search` command](/docs/reference/cli/search).

You can manually create a snapshot of your cloud infrastructure graph using the [`graph snapshot`](/docs/reference/cli/graph/snapshot) command and get a list of snapshots using [`graph list`](/docs/reference/cli/graph/list).

[Use a snapshot in a search](/docs/reference/search#searching-snapshots) using the `graph=<snapshot-name>` filter on the [`search` command](/docs/reference/cli/search) or by adding the `--at` flag with a timestamp (e.g., `--at 2021-06-20T12:00:00Z`) or time delta (e.g., `--at 1w`).
...
Rewind also gives you the power to export an [asset inventory graph](/docs/concepts/asset-inventory-graph) snapshot into a single file using the [`graph export` command](/docs/reference/cli/graph/export).

You can then [import](/docs/reference/cli/graph/import) it back into the system, archive it, or import into another Resoto instance—your data is always accessible and portable.
...
- [`search` Command](/docs/reference/cli/search)
- [`graph` Command](/docs/reference/cli/graph)
