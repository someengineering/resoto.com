...
3. **Automation**
   ...
   After resolving an issue, you can prevent it from recurring by creating a [job](/docs/concepts/automation#jobs) using the [`job add` command](/docs/reference/cli/action-commands/job/add) with the same search parameters. This job will run automatically whenever there are changes in your infrastructure.
