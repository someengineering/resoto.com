...
**Resoto maintains a record of your resources in an [asset inventory graph](/docs/concepts/asset-inventory-graph).** Resoto [scrapes your infrastructure at regular intervals](/docs/concepts/cloud-data-sync) and tracks the configuration of collected resources. Whenever a change to a resource is detected, Resoto updates its internal representation of the resource and captures the change as an event in its graph database:
