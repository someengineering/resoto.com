...
date: 2022-12-23
authors: [matthias]
tags: [Cloud Asset Inventory, Resoto CLI, Graph Database]
description: Resoto maintains a record of your resources in its asset inventory graph.
---
...
**Resoto maintains a record of your resources in its [asset inventory graph](/docs/concepts/asset-inventory-graph).** Resoto [scrapes your infrastructure at regular intervals](/docs/concepts/cloud-data-sync) and tracks the configuration of collected resources. Whenever a change to a resource is detected, Resoto updates its internal representation of the resource and captures the change as an event in its graph database:
...
The [`history` command](/docs/reference/cli/history) in Resoto supports filtering of events by type and time:
...
To retrieve the configuration of the deleted resource, we can pipe the result into the [`dump` command](/docs/reference/cli/dump). Since the complete configuration is part of any event, we can use this approach for any event, no matter which type of event or which time the resource changed.
...
You can not only apply filters, but you can also use any other Resoto command (e.g., [`count`](/docs/reference/cli/count) and [`aggregate`](/docs/reference/cli/aggregate)).
...
We can even aggregate by resource owner and event type if you have a [tagging policy](../effective-cloud-management-tagging-policies/index.mdx) in place where resources have `owner` tags:
...
We plan to continue improving upon and extending the capabilities of the [`history` command](/docs/reference/cli/history).
