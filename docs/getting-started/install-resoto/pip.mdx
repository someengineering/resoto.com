...
2.  Install ArangoDB following the [installation instructions](https://arangodb.com/docs/stable/getting-started-installation.html) for your Linux distribution. Also read the [Linux Operating System Configuration](https://arangodb.com/docs/stable/installation-linux-osconfiguration.html) guide for optimal database performance.
    ...
    See [Security ⇒ Custom Certificates](../../reference/security/index.mdx#custom-certificates) for details on how to generate certificates and encrypt the connection between Resoto Core and the graph database.
...
3.  Create multiple shells/tabs and run each component in a separate shell:
    ...
    [Resoto Core](../../reference/components/core.mdx) only listens on `localhost:8900` by default. [Resoto Core](../../reference/components/core.mdx) can be [configured to listen on all interfaces](../../reference/configuration/core.mdx#network-interfaces) if desired.
