...
2. Install the Discord Infrastructure App
...
4. Now that we've defined the alert trigger, we will simply pipe the result of the search query to the `discord` [custom command](../../../reference/cli/index.mdx#custom-commands), replacing the `title` with your desired alert text and `webhook` with your Discord webhook URL:
...
5. Finally, we want to automate checking of the defined alert trigger and send alerts to Discord whenever the result is true. We can accomplish this by creating a [job](../../../concepts/automation/index.mdx#jobs):
...
A default webhook URL can be specified in the `discord` app configuration, so that you can simply execute `discord --title="..."`.
