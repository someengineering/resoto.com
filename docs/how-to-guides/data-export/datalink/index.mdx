...
:::tip When running Resoto Shell locally, SQLite databases will be downloaded to the local file system. :::

## Exporting Data to a SQLite File
...
This will give you a list of all the tables Resoto created. You can now query the tables and inspect the data:
...
:::note The above query uses the [JSON extension](https://www.sqlite.org/json1.html) to query the JSON data in the `group_ip_permissions` column. Databases like MySQL and PostgreSQL have similar extensions but the syntax will be slightly different. :::
