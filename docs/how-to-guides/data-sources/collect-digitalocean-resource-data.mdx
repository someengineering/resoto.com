...
The [DigitalOcean](../../reference/unified-data-model/digitalocean.mdx) collector is configured within the [Resoto Worker configuration](../../reference/configuration/index.mdx) via the [`config` command](../../reference/cli/setup-commands/configs/index.mdx) in [Resoto Shell](../../reference/components/shell.mdx).
...
**Instead of specifying API tokens or secret access keys in the [Resoto Worker configuration](../../reference/configuration/index.mdx) directly, it is possible to define them using the [`--override` flag or `RESOTOWORKER_OVERRIDE` environment variable](../../reference/configuration/index.mdx#overriding-individual-properties).**
