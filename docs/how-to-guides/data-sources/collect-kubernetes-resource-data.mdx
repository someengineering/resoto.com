...
The [Kubernetes](../../reference/unified-data-model/kubernetes.mdx) collector is configured within the [Resoto Worker configuration](../../reference/configuration/index.mdx) via the [`config` command](../../reference/cli/setup-commands/configs/index.mdx) in [Resoto Shell](../../reference/components/shell.mdx).
...
By default, Resoto performs resource collection each hour. To immediately trigger a collect run, use the [`workflow run` command](../../reference/cli/action-commands/workflows/run.mdx) in [Resoto Shell](../../reference/components/shell):
