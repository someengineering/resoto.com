...
1. Execute the [`search` command](../../../reference/cli/search-commands/search.mdx) in [Resoto Shell](../../../reference/components/shell.mdx) to get the resource for which we want to see the blast radius:
...
3. Export the results in [Graphviz DOT format](https://graphviz.org/doc/info/lang.html):
   ...
   The resulting file is outputted to the [Resoto Shell](../../../reference/components/shell.mdx) download directory, which defaults to the current working directory. Use a [Graphviz](https://graphviz.org) tool to visualize the results:
