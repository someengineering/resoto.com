...
| ------------------------------- | ---------------------------------------------------------------------- | --------- | ------------- |
| `--common-name <name>`          | Server name protected by the SSL certificate.                          | ✔️        |               |
| `--dns-names <dns_names>`       | Space-delimited DNS names that the certificate should be valid for     | ❌        |               |
| `--ip-addresses <ip_addresses>` | Space-delimited IP addresses that the certificate should be valid for. | ❌        |               |
| `--days-valid <days>`           | Number of days the certificate should be valid                         | ❌        | 365           |
