...
For security reasons, [Resoto Core](../components/core.mdx) only listens on the loopback interface by default. This can be changed via the `resotocore.api.web_hosts` option in the `resoto.core` configuration.

Execute the following command in [Resoto Shell](../components/shell.mdx) to open the relevant configuration for editing:
...
Alternatively, you can use the [`config set` command](../cli/setup-commands/config/set.mdx):
...
[Resoto Rewind](/rewind) allows you to set up automated snapshots. You can specify the number of snapshots to retain per label and the associated schedule.
