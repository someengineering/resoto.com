...
Resoto uses an internal configuration system for its [components](../components/index.mdx). Configuration is maintained within [Resoto Core](../components/core.mdx) and can be edited using [Resoto Shell](../components/shell.mdx) using the [`config edit` command](../cli/setup-commands/config/edit.mdx).
...
Instead of editing a component's configuration in a text editor, it is also possible to set the values of specific properties using the [`config set` command](../cli/setup-commands/config/set.mdx).
