...
The [`count` command](../../reference/cli/search-commands/count.mdx) is actually a special type of aggregation.
...
The [`aggregate` command](../../reference/cli/search-commands/aggregate.mdx) tells Resoto to aggregate the search results based on the defined criteria. Each result of the search is then passed to the defined aggregation function(s).

The above example using the [`count`](../../reference/cli/search-commands/count.mdx) could also be rewritten with `aggregate` like so:
