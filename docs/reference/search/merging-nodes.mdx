...
Most cloud resources are maintained in an account. Accounts are modeled as [nodes](../../concepts/asset-inventory-graph/index.mdx#nodes) in Resoto.
...
It is possible to access every property of every parent or child resource in this way. You can use this notation anywhere a property path is specified: in [filters](./filters.mdx), [aggregations](./aggregation.mdx), [`with` clauses](./with-clause.mdx), and [CLI commands](../../reference/cli/index.mdx). Resoto is clever enough to create nested searches on demand when it encocunters this notation.
