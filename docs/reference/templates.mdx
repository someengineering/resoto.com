...
[\{\{ mustache \}\}](https://mustache.github.io) templates can be used to define [custom commands](./cli/index.mdx#custom-commands) and [search templates](./cli/template/index.mdx).

The [`template test` command](./cli/template/test.mdx) offers a way to test templates. We use this functionality below to illustrate template usage.
