...
description: ELT describes the process of extracting raw data from a source, loading it into a destination, and then transforming the data for analytics purposes. The cloud has driven down the cost of compute and storage so that it no longer makes sense to pre-aggregate data in an external processing layer, resulting in the shift away from ETL to ELT.
---

# ELT for Cloud Infrastructure Data
...
<YoutubeEmbed id="5ALXtsUKqeY" title={metadata.title} date={metadata.date} description={metadata.description} />
