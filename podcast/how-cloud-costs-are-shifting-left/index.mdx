...
description: Procurement mechanisms to lower the price point of a cloud resource include reserved instances, enterprise discount programs, savings plans, etc. However, Finance lacks the context that developers have when deploying resources, while developers lack visibility into resource prices and the cost of their deployments. Infracost provides cloud cost estimates for Terraform in pull requests to close this gap and show engineering teams how code changes affect their cloud bills.
---

# How Cloud Costs Are Shifting Left
...
<YoutubeEmbed id="w2HxugqiLlw" title={metadata.title} date={metadata.date} description={metadata.description} />
