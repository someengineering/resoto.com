...
</div></div><ul style={{"marginLeft":"1rem"}}><SchemaItem collapsible={false} name={"message_type"} required={false} schemaDescription={"The name of the action to listen to"} schemaName={"string"} qualifierMessage={undefined} defaultValue={undefined}></SchemaItem><SchemaItem collapsible={false} name={"wait_for_completion"} required={false} schemaDescription={"If an action is sent to this subscriber, the event sender should wait until this\nsubscriber has processed the message and send a done message.\nThis is the expected behaviour of every actor and defaults to true.\n"} schemaName={"boolean"} qualifierMessage={undefined} defaultValue={true}></SchemaItem><SchemaItem collapsible={false} name={"timeout"} required={false} schemaDescription={"This is the duration in seconds this subscriber has time to execute the action.\nAfter this time the sender assumes a failure and rejects the result of this actor.\n"} schemaName={"number"} qualifierMessage={undefined} defaultValue={60}></SchemaItem></ul></details></TabItem></MimeTabs><div><ApiTabs><TabItem label={"200"} value={"200"}><div>
