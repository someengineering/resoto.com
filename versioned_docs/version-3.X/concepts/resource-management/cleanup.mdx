...
**By default, [Resoto Worker](../../reference/components/worker.mdx) does not delete resources marked for deletion.**

Resources marked with `| clean` will stay this way without getting deleted unless [cleanup is enabled](#enabling-cleanup).

Cleanup can be performed manually in [Resoto Shell](../../reference/components/shell.mdx), automatically using a [cleanup plugin](../../reference/components/plugins/index.mdx), or via a scheduled [job](../automation/index.mdx#jobs).
...
To enable cleanup, execute the following command in [Resoto Shell](../../reference/components/shell.mdx) to open the [Resoto Worker](../../reference/components/worker.mdx) configuration for editing:
