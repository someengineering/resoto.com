...
4. Now that we've defined the action and decided on the trigger, create the job using the [`jobs add` command](../../reference/cli/action-commands/job/add.mdx) (replace the value of the `--id` parameter with a unique job identifier):
