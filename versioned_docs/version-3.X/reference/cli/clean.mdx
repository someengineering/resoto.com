...
The `clean` command marks resources for cleaning by setting `desired.clean=true` to the resources. Resources marked as such will be removed during the [`cleanup` phase](../workflows/index.mdx#cleanup) of the next [`collect_and_cleanup` workflow](../workflows/index.mdx#collect_and_cleanup-workflow) run.
