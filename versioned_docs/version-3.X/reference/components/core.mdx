...
Within Resoto Core, there are workflows consisting of steps that result in actions like `collect`, `cleanup` or `generate_metrics`. These actions are received by components like [Resoto Worker](./worker.mdx) and [Resoto Metrics](./metrics.mdx).
...
The [Resoto Core API](../api/index.mdx) is exposed at `https://<resoto-address>:8900/api-doc`. You can also access it at [`https://resoto.com/docs/reference/api`](../api/index.mdx).

Resoto Core has two [API](../api/index.mdx) endpoints to connect to for CLI purposes:
...
Below is a simulation of sending a [Resoto Shell](./shell.mdx) [search](../search/index.mdx) to the [API](../api/index.mdx).
...
Resoto Core is the central hub for everything Resoto does. You can explore additional API endpoints at `https://<resoto-address>:8900/api-doc` or [`https://resoto.com/docs/reference/api`](../api/index.mdx).
