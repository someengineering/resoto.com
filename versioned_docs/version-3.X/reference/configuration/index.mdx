...
Resoto uses an internal configuration system for its [components](../components/index.mdx). Configuration is maintained within [Resoto Core](../components/core.mdx) and can be edited using [Resoto Shell](../components/shell.mdx) using the [`config edit` command](../cli/config/edit.mdx).
