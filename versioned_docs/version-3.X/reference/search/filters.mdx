...
Resoto will add a synthetic `id` property to each [node](../../concepts/asset-inventory-graph/index.mdx#nodes) that is globally unique, no matter which cloud provider you are using. Nodes can be selected by their internal ID via the `id(xyz)` function.
