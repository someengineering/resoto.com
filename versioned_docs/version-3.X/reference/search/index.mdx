...
Alternatively, you can provide the `--at` flag to the [`search` command](../cli/search.mdx) to search at a specific point in time. Resoto will find the [asset inventory graph](../../concepts/asset-inventory-graph/index.mdx) snapshot that is closest to the specified time.
