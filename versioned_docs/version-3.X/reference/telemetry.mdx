...
- Number of started [`resotocore`](./components/core.mdx) instances
- Number of long-running [`resotocore`](./components/core.mdx) instances (greater than one hour)
- Number of executed [CLI commands](./cli/index.mdx)
- Number of executed [jobs](../concepts/automation/index.mdx)
- Number and type of [configured cloud providers](../getting-started/configure-resoto/index.mdx) as well as with configuration change events
- [Installation method](../getting-started/install-resoto/index.mdx)
...
Telemetry can be disabled via the `resotocore.runtime.usage_metrics` [Resoto Core configuration](./configuration/core.mdx) option.
